...
export async function registerClinicalCareRoutes(app: FastifyInstance, prisma: PrismaClient) {
  ...
  app.get("/api/v1/appointments", async (request) => {
    ...
    return { data: await prisma.appointment.findMany({ where: { ...(q.patientId ? { patientId: q.patientId } : {}), ...(q.doctorId ? { doctorId: q.doctorId } : {}), ...(q.status ? { status: q.status.trim().toUpperCase() as any } : {}), ...(q.from || q.to ? { scheduledAt: { ...(q.from ? { gte: new Date(q.from) } : {}), ...(q.to ? { lte: new Date(q.to) } : {}) } } : {}) }, include: { patient: true, doctor: { include: { user: { select: { email: true } }, department: true } }, department: true }, orderBy: { scheduledAt: "asc" }, take: 500 }) };
  });
  ...
  app.post("/api/v1/appointments", async (request, reply) => {
    ...
    if (![b.patientId, b.doctorId, b.departmentId, b.scheduledAt].every(text)) return reply.code(400).send({ error: "patientId, doctorId, departmentId and scheduledAt are required" });
    ...
    const [patient, doctor, department, conflict] = await Promise.all([prisma.patient.findUnique({ where: { id: b.patientId } }), prisma.doctor.findUnique({ where: { id: b.doctorId } }), prisma.department.findUnique({ where: { id: b.departmentId } }), prisma.appointment.findFirst({ where: { doctorId: b.doctorId, scheduledAt, status: { notIn: ["CANCELLED", "NO_SHOW"] as any } } })]);
    if (!patient) return reply.code(404).send({ error: "Patient not found" });
    if (!doctor) return reply.code(404).send({ error: "Doctor not found" });
    ...
    return reply.code(201).send(await prisma.appointment.create({ data: { patientId: b.patientId!, doctorId: b.doctorId!, departmentId: b.departmentId!, scheduledAt }, include: { patient: true, doctor: { include: { department: true } }, department: true } }));
  });
  ...
  app.patch("/api/v1/appointments/:id", async (request, reply) => {
    ...
    try { return await prisma.appointment.update({ where: { id }, data: { ...(status ? { status: status as any } : {}), ...(b.scheduledAt ? { scheduledAt: new Date(b.scheduledAt) } : {}) }, include: { patient: true, doctor: { include: { department: true } }, department: true } }); } catch { return reply.code(404).send({ error: "Appointment not found" }); }
  });
